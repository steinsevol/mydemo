...
<%
   ...
   if(rs.next()){
	   //更新用户信息表
     String strUpdate="update users set password='"+newpw1+
                     "' where username='"+username+"'";
     rst.doExecute(strUpdate);
     out.println("<script lanage='javascript'>alert('密码修改成功');window.location.href='userexit.jsp';</script>");
     
   }else{
     out.println("<script lanage='javascript'>alert('用户名或密码错误');window.location.href='usermanager.jsp';</script>");
   }
%>
